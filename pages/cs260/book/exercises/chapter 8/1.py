...
class DFSGraph(Graph):
    """Implements a DFS graph."""
    def __init__(self):
        """Initializes a DFS graph."""
        super().__init__()
        self.time = 0
    ...
    def dfs(self):
        ...
        for aVertex in self:
            ...
            aVertex.setPred(-1)
            print(aVertex.getId() + " is white ")
        for aVertex in self:
            if aVertex.getColor() == 'white':
                self.dfsvisit(aVertex)

    def dfsvisit(self,startVertex):
        """Function for individually visiting the nodes in depth first search."""
        startVertex.setColor('gray')
        print(startVertex.getId() + " is gray")
        self.time += 1
        startVertex.setColor('black')
        print(startVertex.getId() + " is black")
