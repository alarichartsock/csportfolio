...
class BinHeap:
    def __init__(self):
        self.heapList = [0]
        self.current_size = 0
        self.nonincreasing = False
        self.nondecreasing = True
        lambda nonincreasing, nondecreasing: nonincreasing < nondecreasing
...
def heapSort(self): 
    if self.nonincreasing == True:
    #non decreasing sort
        sortedlist = []
        while self.current_size != 1:
            self.heapList[1], self.heapList[(len(self.heapList) - 1)] = self.heapList[(len(self.heapList) - 1)], self.heapList[1]
